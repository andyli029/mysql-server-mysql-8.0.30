...
bool ColumnBinEncoder::EncoderText_UTF::SecondColumn(vcolumn::VirtualColumn *vc2) {
  if (vc_type.IsString() && vc2->Type().IsString() && collation.collation != vc2->GetCollation().collation) {
    rc_control_.lock(vc2->ConnInfo()->GetThreadID()) << "Nontrivial comparison: " << collation.collation->csname << " with "
                                                   << vc2->GetCollation().collation->csname << system::unlock; // stonedb8
    return false;
  }
